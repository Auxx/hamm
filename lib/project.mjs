...
const defaultConfig = {
  vars: {},
  replaceOnlyIn: [ '.yaml', '.yml' ]
};

export async function getVersion(scriptPath) {
  const pkg = JSON.parse((await fs.readFile(path.join(scriptPath, 'package.json'))).toString());
  return pkg.version;
}
...
export function getTemplateDir(scriptPath) {
  return path.join(scriptPath, 'template');
}
